...
def split_train_val(input_folder, create_folder, train_folder_name, val_folder_name, test_size=0.1, input_image_format=".jpg", seed=0):
    """
    ...
    Mention input_image_format and test_size if the defaults doesn't work
    """
    if not os.path.exists(create_folder):
        os.makedirs(create_folder)

    files = glob.glob("TopoData299"+"/*/")
    ...
    for i in tqdm(range(len(files))):
        x = files[i]
        train, test = train_test_split(x, test_size=0.1, random_state = seed)
...
if __name__ == '__main__':
    for i in range(10):
        split_train_val("train", "training_data_"+str(i), "train", "val", seed=i)
