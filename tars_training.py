...
def train_model(model, dataloaders, dataset_sizes, criterion, optimizer, scheduler, use_gpu, num_epochs=25, mixup = False, alpha = 0.1):
    ...
    for mydata in tqdm(dataloaders['train']):
      print(mydata[1])
    print('Validation Labels')
    for mydata in tqdm(dataloaders['val']):
      print(mydata[1])
    # pdb.set_trace()
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                print("THIS IS TRAINING LOSS: ", trainloss)
                print("THIS IS TRAINING ACCURACY: ", trainacc)

            elif phase == 'val':
                valloss.append(epoch_loss)
                valacc.append(epoch_acc)
                print("THIS IS VALIDATION LOSS: ", valloss)
                print("THIS IS VALIDATION ACCURACY: ", valacc)
...
def model_evaluation(mode, model_conv, input_data_loc, input_shape, use_gpu, name):
    ...
    for img, label in dataloaders:
        ...
        print("bs",bs, "ncrops ", ncrops, "c", c, "h", h, "w", w)
        output = model_conv(inputs.view(-1, c, h, w)) # fuse batch size and ncrops -- error occurs on this line
